...
void bq27541_force_update_current(bool enable)
{
	if (likely(bq27541_registered)) {
		if (atomic_read(&bq27541_di->suspended) != 1) {
			...
			queue_delayed_work(system_power_efficient_wq,
				&bq27541_di->battery_soc_work, msecs_to_jiffies(1));
		}
	}
}
#endif
		...
		const struct i2c_device_id *id)
{
	...
	queue_delayed_work(system_power_efficient_wq, &di->hw_config,
		BQ27541_INIT_DELAY);
	queue_delayed_work(system_power_efficient_wq, &di->battery_soc_work,
		BATTERY_SOC_UPDATE_MS);
